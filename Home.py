...
def process_resume(resume_text, job_description, role_level, department=None):
    ...
    try:
        ...
        if is_senior:
            ...
            exp_bonus = min(100, exp_years * 10)
        elif is_junior:
            weights = {
                'ai_score': 0.3,
                'keyword_score': 0.4,
...
def process_resume(resume_text: str, job_description: str, role_level: str, department: str = None) -> Dict[str, Any]:
    """Simplified and more reliable resume processing"""
    try:
        if not resume_text or not job_description:
            return {
                'score': 0,
                'ai_analysis': {'scores': {'overall': 0, 'detailed': {}}},
                'ats_score': 0,
                'detailed_analysis': 'No content to analyze'
            }
        ai_results = ai_analyzer.parallel_analysis(resume_text, job_description, role_level)
        overall_score = ai_results.get('scores', {}).get('overall', 50)
        detailed_scores = ai_results.get('scores', {}).get('detailed', {})
        ...
        return {
            ...
            'ats_score': detailed_scores.get('keyword', 50),
...
def analyze_resume_with_jd(resume_text: str, job_description: str, role_level: str) -> str:
    """Simplified analysis text generation"""
    try:
